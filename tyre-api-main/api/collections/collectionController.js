...
const getCollections = asyncHandler(async (req, res) => {
  try {
    ...
    const count = await Collection.countDocuments({ ...searchParams });
    const collections = await Collection.find({ ...searchParams })
      .limit(pageSize)
      .populate('parent_collection')
...
const getCollectionById = asyncHandler(async (req, res) => {
  try {
    ...
    if (collection && collection.published_status === 'PUBLISHED') {
      res.json(collection);
    } else {
      res.status(404);
...
const updateCollection = asyncHandler(async (req, res) => {
  try {
    ...
    if (data) {
      ...
      const updatedCollection = await data.save();
