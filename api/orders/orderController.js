...
const getOrderById = asyncHandler(async (req, res) => {
  try {
    ...
    // If not found by _id, try finding by numeric order_id
    if (!order) {
...
const updateOrder = asyncHandler(async (req, res) => {
  try {
    ...
    // MANDATORY Isolation: Vendors can only update THEIR orders
    if (req.user && req.user.role === 'VENDOR') {
      const isVendorOrder = data.products.some(p => p.vendor && p.vendor.toString() === req.user._id.toString());
      if (!isVendorOrder) {
        res.status(401);
