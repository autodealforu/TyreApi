  ...
  try {
    ...
    if (data) {
      ...
      Object.keys(feed).forEach((key) => {
        if (key === 'vendor' && typeof feed[key] === 'object') {
          // Explicitly update sub-fields in the vendor object 
          // while skipping read-only MongoDB fields like _id
          const forbidden = ['_id', '__v', 'createdAt', 'updatedAt'];
          if (!data.vendor) data.vendor = {};
          
          Object.keys(feed.vendor).forEach((vKey) => {
             if (!forbidden.includes(vKey)) {
                data.vendor[vKey] = feed.vendor[vKey];
             }
          });
          data.markModified('vendor');
        } else if (key !== 'password' || (key === 'password' && feed[key])) {
          // Update other fields (password is handled by pre-save hook)
          data[key] = feed[key];
        }
      });
...
// @desc    Get store counts by state
// @route   GET /api/vendors/stats/by-state
// @access  Public
const getStoreStatsByState = asyncHandler(async (req, res) => {
  try {
    const stats = await User.aggregate([
      { $match: { role: 'VENDOR', published_status: 'PUBLISHED' } },
      { $unwind: '$vendor.pickup_address' },
      {
        $group: {
          _id: '$vendor.pickup_address.state',
          count: { $sum: 1 },
        },
      },
      {
    ...
    // Convert to a simple object { "Maharashtra": 5, ... }
    const result = stats.reduce((acc, curr) => {
      if (curr.state) {
        acc[curr.state] = curr.count;
      }
      return acc;
    }, {});
    ...
    console.log(error);
    res.status(502);
    throw new Error('Could not fetch store stats');
  }
});
